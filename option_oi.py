...
def filter_chain(filename, atm_slicer, ind):
    ...
    w = Label(win, text ='Open interest analysis from Option chain API', font = "50",fg="blue") 
    w.pack()
    ...
    s = Label(win, text =f'{ind} Spot : {spot}', font = "50",fg="brown") 
    s.pack()
    ...
    def selected():
        ...
        axs[0].bar(f_strikes,call_oi,color='green',width=45,edgecolor='black',linewidth=1, label=f'CALL OI : {calls}M')
        axs[0].bar(f_strikes,put_oi,color='red',width=30,edgecolor='black',linewidth=1, label=f"PUT OI : {puts}M")
        axs[0].bar(atm,(atm_oi_yval+10000),color='blue',label=f'ATM Strike {atm}', linestyle='dashed', linewidth=1, width=7)
        axs[0].annotate(f'ATM Strike {atm}',xy=(atm+50, atm_oi_yval),xycoords='data')
        axs[0].set_title('Open Interest')
        
        #plotting change in OI and atm strike as another sub plot
        axs[1].bar(f_strikes,call_change_oi,color='green',width=45,edgecolor='black',linewidth=1, label=f'CALL OI change : {calls_change}M')
        axs[1].bar(f_strikes,put_change_oi,color='red',width=30,edgecolor='black',linewidth=1, label=f"PUT OI change : {puts_change}M")
        axs[1].bar(atm,(atm_change_oi_yval+2000),color='blue',label=f'ATM Strike {atm}', linestyle='dashed', linewidth=1, width=7)
        ...
        axs[1].set_title('Change in Open Interest')
    ...
    drop= OptionMenu(win, menu, *expiry_list)
    drop.config(bg='lightblue')
    drop.pack()

    l = Button(win, text='Load', command=selected)
    l.pack()
